...
class Email(models.Model):
    ...
    def open_rate(self):
        totle_sent = self.email_list.count_emails()
        opened_count = EmailTracking.objects.filter(email=self,opened_at__isnull=False).count()

        open_rate = (opened_count/totle_sent) * 100 if totle_sent>0 else 0 
    ...
    def click_rate(self):
        ...
        if opened_count>0:
            ...
            click_rate = (clicked_count/opened_count) * 100 if totle_sent>0 else 0
        else:
            click_rate = 0
...
class Sent(models.Model):
    ...
    totle_sent = models.IntegerField()

    def __str__(self): 
        return str(self.email) + ' - ' + str(self.totle_sent) + 'email_sent'
